...
static HRESULT DeleteKeyRecursive(HKEY hParent, const wchar_t* subkey) {
    ...
    if (result == ERROR_FILE_NOT_FOUND) return S_OK;  // Already gone, that's fine
    return (result == ERROR_SUCCESS) ? S_OK : HRESULT_FROM_WIN32(result);
}
...
STDAPI DllRegisterServer() {
    ...
    //
    // SAPI discovers voices by enumerating keys under:
    //   HKLM\SOFTWARE\Microsoft\Speech\Voices\Tokens\
    //
    // Each key is a "voice token" with:
    //   - CLSID value → which COM class to instantiate
    //   - Custom values → voice-specific settings
    //   - Attributes subkey → display info for voice pickers
    // -----------------------------------------------------------------------
    const std::wstring tokensRoot = L"SOFTWARE\\Microsoft\\Speech\\Voices\\Tokens";

    for (size_t i = 0; i < g_voiceCount; ++i) {
        const VoiceDefinition& voice = g_voices[i];

        std::wstring tokenPath = tokensRoot + L"\\" + voice.tokenName;
    ...
    VLOG(L"DllRegisterServer completed: %zu voices registered", g_voiceCount);
    return S_OK;
}
...
STDAPI DllUnregisterServer() {
    ...
    // -----------------------------------------------------------------------
    // Step 2: Remove all voice tokens
    // -----------------------------------------------------------------------
    const std::wstring tokensRoot = L"SOFTWARE\\Microsoft\\Speech\\Voices\\Tokens";

    for (size_t i = 0; i < g_voiceCount; ++i) {
        std::wstring tokenPath = tokensRoot + L"\\" + g_voices[i].tokenName;
        DeleteKeyRecursive(HKEY_LOCAL_MACHINE, tokenPath.c_str());
        VLOG(L"Unregistered voice: %s", g_voices[i].tokenName);
    }
